...
@test "($PLUGIN_COMMAND_PREFIX:info) success" {
  ...
  assert_contains "${lines[*]}" "clickhouse://l:$password@dokku-clickhouse-l:9000/l"
}
...
@test "($PLUGIN_COMMAND_PREFIX:info) replaces underscores by dash in hostname" {
  ...
  assert_contains "${lines[*]}" "clickhouse://test_with_underscores:$password@dokku-clickhouse-test-with-underscores:9000/test_with_underscores"
...
@test "($PLUGIN_COMMAND_PREFIX:info) success with flag" {
  ...
  local password="$(sudo cat "$PLUGIN_DATA_ROOT/l/PASSWORD")"
  assert_output "clickhouse://l:$password@dokku-clickhouse-l:9000/l"
